...
StackedAreaChart.prototype.initVis = function(){
	...
	vis.width = 635 - vis.margin.left - vis.margin.right;
    vis.height = 400 - vis.margin.top - vis.margin.bottom;


  // SVG drawing area
	vis.svg = d3.select("#" + vis.parentElement).append("svg")
	    .attr("width", vis.width + vis.margin.left + vis.margin.right+20)
    ...
    vis.yAxis = d3.axisLeft()
        .scale(vis.y)
        .tickFormat( function(d) { return numberWithCommas("$ " + d + " million") } );
...
// Draw the layers
    ...
    categories.enter().append("path")
        ...
        .on("mouseover", function (d,i) {
            ...
            socket2 = this;
            console.log(socket2);
            console.log(socket2.getAttribute("indicator"));
            d3.select(".labelStacked").text(dataCategories[i]);
            })
        .on("mouseout", function(d,i) {
            d3.select(".labelStacked").text("");
        });


    // TO-DO: Update tooltip text
    //focus.select(".labeltop").text(d.categories);
